...
final class UploadSrtUrlForFile extends AbstractApi implements VideoStreamingServiceMethod
{
    use CustomHeaders;
    ...
    public function handle(array $data = []): array
    {
        ...
        $postParamsArray = [
            ...
            'srt_url'   => $data[2] ?? $data['lang'],
        ];

        return $this->post(
            Vidoza::API_ENDPOINT . '/files/' . ($data[0] ?? $data['fileCode']) . '/sub-url',
            $postParamsArray
        );
    }
}